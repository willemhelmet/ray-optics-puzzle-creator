...
import type { Point, TouchArea, VirtualObject, VirtualRoom } from "./types";
...
export class P5Renderer {
  ...
  private render() {
    ...
    // Draw virtual rooms (dashed rectangles)
    p.push();
    p.noFill();
    p.strokeWeight(1);
  ...
  private drawVirtualObject(vObj: VirtualObject) {
    if (vObj.sourceType === "triangle") {
      this.drawTriangle(vObj.position, true, vObj.opacity);
    } else if (vObj.sourceType === "viewer") {
      this.drawViewer(vObj.position, true, vObj.opacity);
    }
  }
